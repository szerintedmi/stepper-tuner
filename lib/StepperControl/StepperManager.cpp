...
bool StepperManager::beginMove(StepperMotor &motor, int direction, RunMode mode)
{
  ...
  long anchor = motor.currentPosition();
...
void StepperManager::stopMotor(StepperMotor &motor, bool aborted)
{
  ...
  if (wasRunning)
  {
    motor.recordLastRun(aborted, millis(), motor.currentPosition());
  }
...
void StepperManager::loop()
{
  ...
  for (auto &motor : motors)
  {
    ...
    if (motor.mode == RunMode::Single)
    {
      if (!moving)
      {
        motor.recordLastRun(false, now, motor.currentPosition());
        motor.mode = RunMode::Idle;
        motor.setSegmentSteps(config.target.steps);
      }
    }
...
StepperState StepperManager::state() const
{
  ...
  for (const auto &motor : motors)
  {
    ...
    if (motor.isAttached())
    {
      ...
      long currentPosition = motor.currentPosition();
      long targetPosition = motor.targetPosition();
      ...
      report.speed = motor.currentSpeedHz();
    }
...
bool StepperManager::driverAwake(uint16_t motorId) const
{
